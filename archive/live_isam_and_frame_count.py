...
def controller(x):
    ...
    for process_x in process_list:
        ...
        try:   
            ...
            total_rows = list(df.shape)
            total_string = str(total_rows[0])
            # print(total_string)
            df_string = pd.DataFrame([total_string], columns=['Frames'])
            df_counts = df['Frames'].value_counts()
            
            df_counts.to_excel(writer, sheet_name= name) # Write Sheet to the XLSX workbook

        except (UnboundLocalError, KeyError):
            print('Not an ProcessID or No Information')
            continue # Use this to pass a Process ID that fails to give informaion
